...
这是介绍Spring框架的书，Spring把DIP(依赖倒置原则)玩活了，IoC(控制反转)和DI(依赖注入),AOP(面向切面编程)发挥到了极致，再加上Java1.5的注解，如虎添翼。本书从Bean,BeanFactory开始，介绍了Spring框架的哲学，改写对依赖的调用方式为配置注入方式，后面介绍了JDBC,Hibernate,Spring事务，单元测试。末尾提到了敏捷开发，以及SOA的一种实现ROP。看完以后，可以对Spring框架有一个总体性的认识。

## **How Tomcat Works**

这本书从Socket开始，逐渐搭建了一个Web服务器，介绍了服务器如何处理HTTP请求，介绍了Tomcat中的Servlet解决方案，介绍了Tomcat中的连接器和容器。由浅入深，从零开始，轮子慢慢丰富和发展起来。包括Tomcat中的默认实现，以及Session处理，安全性处理，日志等内容。如果想Tomcat原理多了解一些，推荐本书。

## **深入Java虚拟机**

对初学者而言，我觉得这本书，是为了理解虚拟机，最好的参考读物了。这本书是讲JVM的，而不是在讲Java，站在JVM的角度看待世界，通透了很多。包括Java为什么是安全的，为什么跨平台的，为什么具有网络移动性，只有站在JVM角度才能更好的理解。JVM的内存模型，类型系统，本书写的很详细。我只看了前半本，后半本可以当做资料查阅一下。
